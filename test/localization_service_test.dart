import 'package:chat2you/app/data/local/my_shared_pref.dart';
import 'package:chat2you/config/translations/localization_service.dart';
...
main() async {
  ...
  test('Check getting/updating current local', () async {
    ...
    await LocalizationService.updateLanguage('pt');
    Locale currentLocaleAfterUpdate = LocalizationService.getCurrentLocal();
    expect(currentLocaleAfterUpdate.languageCode, 'pt');
  });
  ...
  test('Check if current language is English', () async {
    ...
    bool isCurrentLangEnglishAfterUpdate =
        LocalizationService.getCurrentLocal().languageCode.contains('pt');
  ...
  testWidgets('Check translation', (tester) async {
    ...
    expect(helloWordAfterChangingLanguage, 'Olá!');
  });
}
