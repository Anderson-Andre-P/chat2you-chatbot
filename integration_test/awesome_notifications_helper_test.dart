...
main() async {
  ...
  testWidgets('Test showing local notification using awesome notifications',
      (tester) async {
    ...
    AwesomeNotificationsHelper.awesomeNotifications.setListeners(
        onActionReceivedMethod:
            MockNotificationController.onActionReceivedMethod,
        onNotificationCreatedMethod:
            MockNotificationController.onNotificationCreatedMethod,
        onNotificationDisplayedMethod:
            MockNotificationController.onNotificationDisplayedMethod,
        onDismissActionReceivedMethod:
            MockNotificationController.onDismissActionReceivedMethod);

    // pump widget
    await tester.pumpWidget(const TestWidget());

    // request permission to show notifications
    bool showNotificationsPermissionGranted = await AwesomeNotificationsHelper
    ...
    Finder generalNotificationsFab =
        find.byKey(const ValueKey('general_notifications_fap'));
    Finder chatNotificationsFab =
        find.byKey(const ValueKey('chat_notifications_fap'));
        ...
        body: SafeArea(
          child: Center(
            child: Obx(() => Column(
                  ...
                  children: [
                    Text(
                        'Notifications Count => ${controller.notificationsCounter.value}'),
                    Text(
                        'Notifications Action => ${controller.notificationAction.value}'),
                  ],
                )),
          ),
        ),
            ...
            FloatingActionButton(
              ...
              onPressed: () async {
                await AwesomeNotificationsHelper.showNotification(
                    title:
                        'test number ${controller.notificationsCounter.value}',
                    body:
                        'test notification number ${controller.notificationsCounter.value}',
...
class MockNotificationController {
  ...
  static Future<void> onActionReceivedMethod(
    ...
    Get.find<TestController>()
        .onNotificationActionClicked(receivedAction.buttonKeyPressed);
  }
}
